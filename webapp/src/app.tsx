  ...
  implements IProjectView
{
          ...
          <>
            ...
            <ul>
              {ghidToBeApproved.map((scr) => (
                <li key={scr.fullName}>
                  <a href={`https://github.com/${scr.project}`}>
                    {scr.fullName}
                  </a>
                </li>
              ))}
  ...
  isBlocksActive(): boolean {
    return (
      ...
      this.editorFile &&
      this.editorFile.name == "main.blocks"
    );
  }
  ...
  openPython(giveFocusOnLoading = true) {
    ...
    if (this.isBlocksActive()) {
      ...
      if (!mainpy) this.updateFileAsync("main.py", "# ...", false);
  ...
  private convertTypeScriptToPythonAsync() {
    ...
    if (cached) {
      convertPromise = this.saveVirtualFileAsync(
  ...
  private editorChangeHandler = Util.debounce(
    () => {
      if (!this.editor.isIncomplete()) {
        ...
        if (this.isTutorial() && pxt.appTarget.appTheme.tutorialCodeValidation)
          this.editor.validateTutorialCode(this.state.tutorialOptions);
      }
  ...
  private initEditors() {
    ...
    let changeHandler = () => {
      ...
      if (
        this.state.simState != pxt.editor.SimState.Stopped &&
        pxt.appTarget.simulator &&
      ...
      onStateChanged: (state) => {
        ...
        switch (state) {
          ...
          case pxsim.SimulatorState.Running:
            this.setState(
              { simState: pxt.editor.SimState.Running },
              simStateChanged
            );
            break;
        }
  ...
  setTutorialStep(step: number) {
    ...
    if (step > -1) {
      ...
      if (pxt.appTarget.appTheme.tutorialCodeValidation)
        this.editor.validateTutorialCode(this.state.tutorialOptions);
    }
  }

  setTutorialCodeStatus(
    step: number,
    status: pxt.tutorial.TutorialRuleStatus[]
  ) {
    const tutorialOptions = this.state.tutorialOptions;
    const stepInfo =
      tutorialOptions.tutorialStepInfo[tutorialOptions.tutorialStep];
    const tutorialCodeValidationIsOn =
      tutorialOptions.metadata.tutorialCodeValidation;
  ...
  async loadHeaderAsync(
    ...
    if (checkAsync) return checkAsync.then(() => this.openHome());
    ...
    if (tryCloudSync) {
      ...
      await Promise.race([
        ...
        cloud.syncAsync([h]).then((changes) => {
          if (changes.length) {
  ...
  private internalLoadHeaderAsync(
    h: pxt.workspace.Header,
    editorState?: pxt.editor.EditorState
  ): Promise<void> {
    ...
    return compiler
      .newProjectAsync()
      .then(() =>
        h.backupRef ? workspace.restoreFromBackupAsync(h) : Promise.resolve()
      )
      ...
      .then(() => {
        ...
        if (!e && h.editor && file.getVirtualFileName(h.editor))
          file =
            main.lookupFile("this/" + file.getVirtualFileName(h.editor)) ||
            file;
        ...
        if (tutorialPreferredEditor) {
          file =
            main.lookupFile(
              "this/" + filenameForEditor(tutorialPreferredEditor)
            ) || file;
        }
        ...
        pkg.mainPkg.getCompileOptionsAsync().catch((e) => {
  ...
  private loadTutorialFiltersAsync(): Promise<void> {
    ...
    return this.loadBlocklyAsync()
      .then(() =>
        tutorial.getUsedBlocksAsync(t.tutorialCode, t.tutorial, t.language)
      )
      .then((tutorialBlocks) => {
      ...
      .catch((e) => {
        ...
        core.errorNotification(
          lf("Oops, an error occured as we were loading the tutorial.")
        );
  ...
  private loadTutorialTemplateCodeAsync(): Promise<void> {
    ...
    if (projectname === pxt.JAVASCRIPT_PROJECT_NAME) {
      ...
      decompilePromise = compiler
        .decompilePythonSnippetAsync(template)
        .then((pyCode) => {
          if (pyCode) {
      ...
      decompilePromise = compiler
        .decompileBlocksSnippetAsync(template)
        .then((resp) => {
  ...
  async createProjectAsync(options: ProjectCreationOptions): Promise<void> {
    ...
    if (options.preferredEditor) cfg.preferredEditor = options.preferredEditor;
        ...
        if (loadBlocks && preferredEditor == pxt.BLOCKS_PROJECT_NAME) {
          return this.createProjectAsync(opts).then(() => {
            return this.loadBlocklyAsync()
              ...
              .then((blocksInfo) =>
                compiler.decompileAsync("main.ts", blocksInfo)
              )
              .then((resp) => {
    ...
    try {
      ...
      try {
        ...
        if (!resp.outfiles[fn]) {
          ...
          const noHexFileDiagnostic = resp.diagnostics?.find(
            (diag) => diag.code === 9043
          );
  ...
  toggleSimulatorCollapse() {
    ...
    if (
      state.simState == pxt.editor.SimState.Stopped &&
  ...
  startSimulator(opts?: pxt.editor.SimulatorStartOptions) {
    ...
    return this.saveFileAsync().then(() =>
      this.runSimulator({ debug: this.state.debugging, clickTrigger })
    );
  }
  ...
  runSimulator(opts: compiler.CompileOptions = {}): Promise<void> {
    ...
    return (() => {
      ...
      return this.setStateAsync({
        simState: pxt.editor.SimState.Starting,
        autoRun: autoRun,
      })
        .then(() =>
          emptyRun
  ...
  hwDebug() {
    ...
    if (
      this.state.simState != pxt.editor.SimState.Running ||
      !simulator.driver.isDebug()
    )
  ...
  blocksScreenshotAsync(
    ...
    if (
      pxt.blocks.layout.screenshotEnabled() &&
      this.blocksEditor &&
      this.blocksEditor.isReady &&
      this.blocksEditor.editor
    )
      return pxt.blocks.layout.screenshotAsync(
        this.blocksEditor.editor,
        pixelDensity,
        encodeBlocks
      );
  ...
  renderBlocksAsync(
    ...
    return compiler
      ...
      .then((resp) => {
        ...
        return {
          svg: svg,
          xml: pxt.blocks.layout.blocklyToSvgAsync(
            svg,
            viewBox[0],
            viewBox[1],
            viewBox[2],
  ...
  anonymousPublishAsync(screenshotUri?: string): Promise<string> {
    ...
    return this.saveProjectNameAsync()
      ...
      .then((files) => {
        ...
        return workspace
          .anonymousPublishAsync(epkg.header, files, meta, screenshotUri)
          .then((inf) => inf.id);
      })
      .finally(() => {
  ...
  updateHeaderNameAsync(name: string): Promise<void> {
    ...
    config.name = name;
    return f
      .setContentAsync(pxt.Package.stringifyConfig(config))
      .then(() => {
  ...
  private loadActivityFromMarkdownAsync(
    path: string,
    title?: string,
    ...
    if (/^\//.test(path)) {
      ...
      p = pxt.Cloud.markdownAsync(path).then((md) => {
        autoChooseBoard = true;
        return processMarkdown(md);
      });
    } else if (scriptId) {
      pxt.tickEvent("tutorial.shared", { tutorial: scriptId });
      p = workspace.downloadFilesByIdAsync(scriptId).then((files) => {
      ...
      p = pxt
        ...
        .then((config) => {
          ...
          return (
            ghid.tag
              ? Promise.resolve(ghid.tag)
              : pxt.github.latestVersionAsync(ghid.slug, config, true)
          ).then((tag) => {
            if (!tag) {
            ...
            return pxt.github.downloadPackageAsync(
              `${ghid.slug}#${ghid.tag}`,
              config
            );
          });
        ...
        .then((gh) => {
          ...
          if (gh.files[pxt.TUTORIAL_INFO_FILE]) {
            p.then(() =>
              pxt.tutorial.parseCachedTutorialInfo(
  ...
  private startTutorialAsync(
    ...
    return this.loadActivityFromMarkdownAsync(
      ...
      .then((r) => {
        ...
        const { options, editor: parsedEditor } =
          pxt.tutorial.getTutorialOptions(
            md,
            tutorialId,
            filename,
            reportId,
        ...
        return this.createProjectAsync({
          ...
          autoChooseBoard
            ? this.autoChooseBoardAsync(features)
            : Promise.resolve()
        );
      })
      .catch((e) => {
  ...
  startActivity(
    activity: pxt.editor.Activity,
    path: string,
    title?: string,
    editorProjectName?: string,
    focus = true,
    opts?: pxt.editor.ExampleImportOptions
  ) {
    switch (activity) {
      case "tutorial":
      ...
      case "recipe":
        ...
        break;
      case "example":
        pxt.tickEvent("example.start", {
          example: path,
          editor: editorProjectName,
        });
        this.importExampleAsync({
          name: title,
          path,
          preferredEditor: editorProjectName,
          ...opts,
        });
        break;
    }
      ...
      return this.exitTutorialAsync()
        ...
        .then(() => {
          if (
            ...
            pxt.tickEvent("tutorial.share", undefined, {
  ...
  private handleFileListRef = (c: HTMLDivElement) => {
    ...
    if (typeof ResizeObserver !== "undefined") {
      const observer = new ResizeObserver(() => {
        ...
        else this.fileListRef.classList.add("invisibleScrollbar");
      });
      observer.observe(c);
    }
  };
  ...
  renderCore() {
    ...
    const flyoutOnly =
      this.state.editorState && this.state.editorState.hasCategories === false;
    ...
    const expandedStyle = inTutorialExpanded
      ? this.getExpandedCardStyle(flyoutOnly)
      : null;
    const invertedTheme =
      ...
      <div id="root" className={rootClasses}>
        ...
        <div id="simulator" className="simulator">
          <div id="filelist" ref={this.handleFileListRef} className="ui items">
            ...
            />
            {this.state.keymap && simOpts.keymap && (
              <keymap.Keymap parent={this} />
            )}
            <div className="ui item portrait hide hidefullscreen">
              {pxt.options.debug ? (
                <sui.Button
                  ...
                  icon="xicon chip"
                  text={"Dev Debug"}
                  onClick={this.hwDebug}
                />
              ) : (
                ""
              )}
            </div>
            {/* AMY: ENERGY VISUALIZER */}
            <div className="ui item portrait">
              <h3> Visualizer</h3>
              <INPUT
            </div>
            ...
            {!isHeadless && (
              <div
                ...
                onClick={this.toggleSimulatorFullscreen}
              ></div>
            )}
          </div>
        </div>
...
function parseLocalToken() {
  const qs = core.parseQueryString(
...
function initSerial() {
  const isHF2WinRTSerial =
    pxt.appTarget.serial && pxt.appTarget.serial.useHF2 && pxt.winrt.isWinRT();
...
function handleHash(
  ...
  switch (newHash.cmd) {
    ...
    case "skillmapimport":
      ...
      editor
        .importSkillmapProjectAsync(headerId)
...
function saveAsBlocks(): boolean {
  try {
    return (
      /saveblocks=1/.test(window.location.href) &&
      !!pkg.mainPkg.readFile("main.blocks")
    );
  } catch (e) {
    return false;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  pxt.options.light =
    /light=1/i.test(window.location.href) ||
  ...
  if (wsPortMatch) {
    ...
    pxt.BrowserUtils.changeHash(
      window.location.hash.replace(wsPortMatch[0], "")
    );
  } else {
  ...
  const importHex = (
    hex: pxt.cpp.HexFile,
    options?: pxt.editor.ImportFileOptions
  ) => theEditor.importHex(hex, options);
  ...
  if (query["ws"]) workspace.setupWorkspace(query["ws"]);
    ...
    (theme.allowParentController || isController) &&
    pxt.BrowserUtils.isIFrame()
  )
  ...
  if (workspace.getWorkspaceType() === "iframe") auth.enableAuth(false);
  Promise.resolve()
    .then(async () => {
      ...
      if (/[&?]translate=1/.test(href) && !pxt.BrowserUtils.isIE()) {
        ...
        const serveLocal =
          ...
          (pxt.BrowserUtils.isLocalHostDev() &&
            (pxt.appTarget.appTheme.defaultLocale || "en") === useLang);
  ...
  document.addEventListener("visibilitychange", (ev) => {
    if (theEditor) theEditor.updateVisibility();
  });
