...
namespace ts.pxtc {
  ...
  export class PxtNode {
    ...
    refresh() {
      ...
      if (
        this.proc &&
        !this.usedActions &&
        !getEnclosingFunction(this.proc.action)
      )
        this.resetEmit();
      else if (this.proc && !this.proc.cachedJS) this.resetEmit();
      else if (this.usedNodes) this.flags |= PxtNodeFlags.FromPreviousCompile;
      if (this.classInfo) this.classInfo.reset();
    }
    ...
    resetEmit() {
      ...
      if (
        this.proc &&
        this.proc.classInfo &&
        this.proc.classInfo.ctor == this.proc
      )
  ...
  function taggedSpecialValue(n: number) {
    return (n << 2) | 2;
  }
  ...
  export function isInPxtModules(node: Node) {
    if (node.pxt) return !!(node.pxt.flags & PxtNodeFlags.InPxtModules);
  ...
  export function pxtInfo(n: Node) {
    if (!n.pxt) {
      ...
      if (isInPxtModules(n)) info.flags |= PxtNodeFlags.InPxtModules;
      n.pxt = info;
      return info;
    } else {
      const info = n.pxt;
      if (info.wave != currNodeWave) {
        ...
        if (!compileOptions || !compileOptions.skipPxtModulesTSC)
          ...
          if (info.flags & PxtNodeFlags.InPxtModules) {
            if (compileOptions.skipPxtModulesEmit) info.refresh();
            else info.resetEmit();
          } else info.resetAll();
        }
      }
  ...
  function isSyntheticThis(def: Declaration) {
    if ((<any>def).isThisParameter) return true;
    else return false;
  }
  ...
  function getBitSize(decl: TypedDecl) {
    ...
    switch ((decl.type as TypeReferenceNode).typeName.getText()) {
      case "int8":
        return BitSize.Int8;
      case "int16":
        return BitSize.Int16;
      case "int32":
        return BitSize.Int32;
      case "uint8":
        return BitSize.UInt8;
      case "uint16":
  ...
  export function sizeOfBitSize(b: BitSize) {
    switch (b) {
      case BitSize.None:
        return target.shortPointers ? 2 : 4;
      case BitSize.Int8:
        return 1;
      case BitSize.Int16:
        return 2;
  ...
  export function setCellProps(l: ir.Cell) {
    ...
    if (!isSyntheticThis(l.def)) {
      ...
      if (l.bitSize != BitSize.None) {
        l._debugType =
          (isBitSizeSigned(l.bitSize) ? "int" : "uint") +
          8 * sizeOfBitSize(l.bitSize);
    ...
    if (l.isLocal() && l.bitSize != BitSize.None) {
      ...
      userError(
        9256,
        lf("bit sizes are not supported for locals and parameters")
      );
    }
  }
  ...
  export function isStatic(node: Declaration) {
    return (
      node &&
      node.modifiers &&
      node.modifiers.some((m) => m.kind == SK.StaticKeyword)
    );
  }

  export function isReadOnly(node: Declaration) {
    return (
      node.modifiers && node.modifiers.some((m) => m.kind == SK.ReadonlyKeyword)
    );
  }

  export function getExplicitDefault(attrs: CommentAttrs, name: string) {
    if (!attrs.explicitDefaults) return null;
    if (attrs.explicitDefaults.indexOf(name) < 0) return null;
  ...
  function getEnclosingMethod(node: Node): MethodDeclaration {
    if (!node) return null;
    if (isClassFunction(node)) return <MethodDeclaration>node;
  ...
  function getEnclosingFunction(node0: Node) {
    ...
    while (true) {
      ...
      if (!node)
        userError(
          9229,
          lf("cannot determine parent of {0}", stringKind(node0))
        );
      switch (node.kind) {
        case SK.MethodDeclaration:
        case SK.Constructor:
        ...
        case SK.SourceFile:
          ...
          if (hadLoop) return _rootFunction;
          return null;
      }
    }
  }
  ...
  function isVar(d: Declaration): boolean {
    if (!d) return false;
    if (d.kind == SK.VariableDeclaration) return true;
    if (d.kind == SK.BindingElement) return isVar(d.parent.parent as any);
    return false;
  }
  ...
  function isGlobalVar(d: Declaration) {
    if (!d) return false;
    return (
      (isVar(d) && !getEnclosingFunction(d)) ||
  ...
  function isParameter(d: Declaration): boolean {
    if (!d) return false;
    if (d.kind == SK.Parameter) return true;
    if (d.kind == SK.BindingElement) return isParameter(d.parent.parent as any);
    return false;
  }

  function isTopLevelFunctionDecl(decl: Declaration) {
    return (
      (decl.kind == SK.FunctionDeclaration && !getEnclosingFunction(decl)) ||
  ...
  function getRefTagToValidate(tp: string): number {
    switch (tp) {
      case "_Buffer":
        return pxt.BuiltInType.BoxedBuffer;
      case "_Image":
        return target.imageRefTag || pxt.BuiltInType.RefImage;
      case "_Action":
        return pxt.BuiltInType.RefAction;
      case "_RefCollection":
        return pxt.BuiltInType.RefCollection;
      default:
        return null;
    }
  }
    ...
    usedMethods() {
      ...
      for (let k of Object.keys(this.methods))
        for (let m of this.methods[k]) {
          ...
          if (info.flags & PxtNodeFlags.IsUsed) r.push(m);
        }
      return r;
    }
  }
  ...
  export function getComments(node: Node) {
    if (node.kind == SK.VariableDeclaration) node = node.parent.parent; // we need variable stmt
  ...
  export function parseComments(node: Node): CommentAttrs {
    ...
    if (pinfo.commentAttrs) return pinfo.commentAttrs;
  ...
  export function getName(node: Node & { name?: any }) {
    if (!node.name || node.name.kind != SK.Identifier) return "???";
  ...
  function genericRoot(t: Type) {
    if (isObjectType(t) && t.objectFlags & ObjectFlags.Reference) {
      ...
      if (r.typeArguments && r.typeArguments.length) return r.target;
    }
    return null;
  }
  ...
  function isArrayType(t: Type) {
    ...
    return (
      t.objectFlags & ObjectFlags.Reference &&
      t.symbol &&
      t.symbol.name == "Array"
    );
  }
  ...
  function isInterfaceType(t: Type) {
    ...
    return (
      !!(t.objectFlags & ObjectFlags.Interface) ||
  ...
  function isClassType(t: Type) {
    ...
    return !!(
      t.objectFlags & ObjectFlags.Class ||
  ...
  function isStructureType(t: Type) {
    return (
      isFunctionType(t) == null &&
      (isClassType(t) || isInterfaceType(t) || isObjectLiteral(t))
    );
  }
  ...
  function castableToStructureType(t: Type) {
    return (
      isStructureType(t) ||
      t.flags & (TypeFlags.Null | TypeFlags.Undefined | TypeFlags.Any)
    );
  }
  ...
  function arrayElementType(t: Type, idx = -1): Type {
    if (isArrayType(t)) return checkType((<TypeReference>t).typeArguments[0]);
  ...
  function typeOf(node: Node) {
    ...
    if (info.typeCache) return info.typeCache;
    if (isExpression(node)) r = checker.getContextualType(<Expression>node);
    if (!r) {
      try {
  ...
  function checkUnionOfLiterals(t: Type): HasLiteralType {
    ...
    u.types.forEach((tp) => {
      if (constituentType === undefined) {
        ...
        else if (tp.flags & TypeFlags.BooleanLike)
          constituentType = HasLiteralType.Boolean;
        else if (tp.flags & TypeFlags.StringLike)
          constituentType = HasLiteralType.String;
        else if (tp.flags & TypeFlags.EnumLike)
          constituentType = HasLiteralType.Enum;
      } else {
        switch (constituentType) {
          case HasLiteralType.Number:
            allGood = allGood && !!(tp.flags & TypeFlags.NumberLike);
            break;
          case HasLiteralType.Boolean:
            allGood = allGood && !!(tp.flags & TypeFlags.BooleanLike);
            break;
          case HasLiteralType.String:
            allGood = allGood && !!(tp.flags & TypeFlags.StringLike);
            break;
          case HasLiteralType.Enum:
            allGood = allGood && !!(tp.flags & TypeFlags.EnumLike);
            break;
        }
      }
    });
  ...
  function checkInterfaceDeclaration(bin: Binary, decl: InterfaceDeclaration) {
    const check = (d: Declaration) => {
      if (d && d.kind == SK.ClassDeclaration)
        userError(
          9261,
          lf("Interface with same name as a class not supported")
        );
    };
    check(decl.symbol.valueDeclaration);
    if (decl.symbol.declarations) decl.symbol.declarations.forEach(check);
        ...
        switch (h.token) {
          case SK.ExtendsKeyword:
            ...
            if (isClassType(tp)) {
              userError(
                9262,
                lf("Extending a class by an interface not supported.")
              );
            }
        }
      }
  ...
  function getTypeParameters(
    fun: FunctionLikeDeclaration | SignatureDeclaration
  ): NodeArray<TypeParameterDeclaration> | TypeParameterDeclaration[] {
    ...
    if (isClassFunction(fun) || fun.kind == SK.MethodSignature) {
      if (
        fun.parent.kind == SK.ClassDeclaration ||
        fun.parent.kind == SK.InterfaceDeclaration
      ) {
  ...
  export function getDeclName(node: Declaration): string {
    ...
    if (!text) {
      if (node.kind == SK.Constructor) {
        ...
        for (
          let parent = node.parent as Declaration;
          parent;
          parent = parent.parent as Declaration
        ) {
  ...
  export function compileBinary(
    ...
    if (compilerHooks.preBinary) compilerHooks.preBinary(program, opts, res);
    ...
    if (opts.target.isNative) {
      ...
      if (extinfo && extinfo.disabledDeps) {
        const enabled = (opts.otherMultiVariants || []).find(
          (e) => e.extinfo && !e.extinfo.disabledDeps
        );
        if (enabled) {
          pxt.debug(
            `using alternative extinfo (due to ${extinfo.disabledDeps})`
          );
    ...
    if (!opts.forceEmit || res.diagnostics.length == 0) {
      ...
      const postProcessing = files.find(
        (sf) => sf.fileName === pxt.TUTORIAL_CODE_STOP
      );
      if (postProcessing) {
    ...
    let mainSrcFile = program
      .getSourceFiles()
      .filter((f) => Util.endsWith(f.fileName, entryPoint))[0];
    ...
    if (resDiags.length == 0) resDiags = diagnostics.getDiagnostics();

    if (compilerHooks.postBinary) compilerHooks.postBinary(program, opts, res);
    ...
    function buildInstructions() {
      ...
      let bufferAddrSize =
        1 +
        bin.varsToCheckpoint.length * 4 +
        bin.boolsToCheckpoint.length +
        bin.arraysToCheckpoint.length * arraylength +
      ...
      for (let i = 0; i < bin.varsToCheckpoint.length; i++) {
        ...
        let write_number_expr = new ir.Expr(
          4,
          [addr_expr, val_expr, addrlength_expr],
          fram_write_number_procid
        );
        let read_number_expr = new ir.Expr(
          4,
          [addr_expr, addrlength_expr],
          fram_read_number_procid
        );
        let assign_read_number_expr = new ir.Expr(
          8,
          [val_expr, read_number_expr],
          undefined
        );
      ...
      for (let i = 0; i < bin.boolsToCheckpoint.length; i++) {
        ...
        let addrlength_expr = new ir.Expr(1, null, valueEncode(bufferAddrSize));
        let write_bool_expr = new ir.Expr(
          4,
          [addr_expr, val_expr, addrlength_expr],
          fram_writeBoolean_procid
        );
        let read_bool_expr = new ir.Expr(
          4,
          [addr_expr, addrlength_expr],
          fram_readBoolean_procid
        );
        let assign_read_bool_expr = new ir.Expr(
          ir.EK.Store,
          [val_expr, read_bool_expr],
          null
        );
      ...
      for (let j = 0; j < bin.arraysToCheckpoint.length; j++) {
        let addr_expr = new ir.Expr(
          1,
          null,
          valueEncode(arrayAddrBegin + j * arraylength)
        );
        let val_expr = new ir.Expr(9, null, bin.arraysToCheckpoint[j]);
        let addrlength_expr = new ir.Expr(1, null, valueEncode(bufferAddrSize));
        let write_array_expr = new ir.Expr(
          4,
          [addr_expr, val_expr, addrlength_expr],
          fram_writeArray_procid
        );
        let arr_type_expr = new ir.Expr(1, null, valueEncode(1)); // for now all types will be number
        let read_array_expr = new ir.Expr(
          4,
          [addr_expr, arr_type_expr, addrlength_expr],
          fram_readArray_procid
        );
        let assign_read_array_expr = new ir.Expr(
          ir.EK.Store,
          [val_expr, read_array_expr],
          null
        );
      ...
      let stringAddrBegin =
        arrayAddrBegin + bin.arraysToCheckpoint.length * 100;
      for (let i = 0; i < bin.stringsToCheckpoint.length; i++) {
        let addr_expr = new ir.Expr(
          1,
          null,
          valueEncode(stringAddrBegin + i * arraylength)
        );
        let val_expr = new ir.Expr(9, null, bin.stringsToCheckpoint[i]);
        let addrlength_expr = new ir.Expr(1, null, valueEncode(bufferAddrSize));
        let write_str_expr = new ir.Expr(
          4,
          [addr_expr, val_expr, addrlength_expr],
          fram_writeString_procid
        );

        let read_str_expr = new ir.Expr(
          4,
          [addr_expr, addrlength_expr],
          fram_readString_procid
        );
        let assign_read_str_expr = new ir.Expr(
          ir.EK.Store,
          [val_expr, read_str_expr],
          null
        );
...
*/
      ...
      //build the total expression
      let canary_expr = new ir.Expr(
        3,
        [expr0, expr1, expr2],
        "BufferMethods::getNumber"
      );
      let canary_set_expr = new ir.Expr(
        3,
        [expr0, expr1, expr2, valexpr1],
        "BufferMethods::setNumber"
      );
      ...
      if (bin.optimization == 1 || bin.optimization == 3) {
        ...
        let opt_store_expr = new ir.Expr(
          8,
          [opt_cellref_expr, val0_expr],
          null
        );
      ...
      let bnot_assign_expr = new ir.Expr(
        8,
        [gencell_expr, bnot_expr],
        undefined
      );
      ...
      let gen_write8_expr = new ir.Expr(
        4,
        [addr_zero_expr, gencell_expr],
        fram_write8_procid
      );
      ...
      for (let glb_var of bin.globals) {
        ...
        if (glb_var._debugType == "number") {
          ...
          for (let i = 0; i < bin.procs[0].body.length; i++) {
            if (bin.procs[0].body[i].stmtKind == ir.SK.Expr) {
              if (bin.procs[0].body[i].expr.exprKind == ir.EK.Store) {
                ...
                if (
                  bin.procs[0].body[i].expr.args[0].data.getName() ==
                  glb_var.getName()
                ) {
          ...
          if (count > 1) {
            ...
            if (
              glb_var.getName() != "timer" &&
              glb_var.getName() != "weight" &&
              glb_var.getName() != "jit" &&
              glb_var.getName() != "gencopy" &&
              glb_var.getName() != "generation" &&
              !glb_var.getName().includes("customMKArg")
            ) {
          ...
          if (
            glb_var.getName() != "_intervals" &&
            glb_var.getName() != "_pollEventQueue"
          ) {
    ...
    function mainBoilerplate(proc: ir.Procedure, start: number, end: number) {
      ...
      let insertRestoreCount =
        bin.varsToCheckpoint.length -
        1 +
        bin.arraysToCheckpoint.length +
        bin.boolsToCheckpoint.length +
        bin.stringsToCheckpoint.length;
      ...
      for (i = start; i < end; i++) {
        if (proc.body[i].stmtKind == 2 || proc.body[i].stmtKind == 3) {
          ...
          if (proc.body[i].expr.exprKind == ir.EK.Store) {
            console.log(
              "found store, " + proc.body[i].expr.args[0].data.getName()
            );
    ...
    function mainRestore(proc: ir.Procedure, start: number, end: number) {
      ...
      for (j = 0; j < bin.checklabel.length; j++) {
        ...
        let numop_expr = new ir.Expr(
          3,
          [label_cell_expr, expr_j],
          "numops::eq"
        );
        let toBool_expr = new ir.Expr(3, [numop_expr], "numops::toBoolDecr");
        let if_stmt = new ir.Stmt(3, toBool_expr);
        ...
        let proc_number = parseInt(
          bin.checklabel[j].lblName.substring(idstart + 1, strlen)
        );
        ...
        let functioncall_expr = new ir.Expr(
          ir.EK.ProcCall,
          [],
          function_w_chk_procid
        );
        ...
        if (proc_number != 1) {
          ...
          let t_numop_expr = new ir.Expr(
            3,
            [label_cell_expr, t_expr_j],
            "numops::eq"
          );
          let t_toBool_expr = new ir.Expr(
            3,
            [t_numop_expr],
            "numops::toBoolDecr"
          );
      ...
      while (1) {
        if (proc.body[i].stmtKind == ir.SK.Label) {
          if (proc.body[i].lblName.includes("final")) {
            ...
            let label_number_expr = new ir.Expr(
              1,
              null,
              valueEncode(bin.checklabel.length)
            );
            let label_cell_expr = new ir.Expr(9, null, bin.label_cell);
            let label_assign_expr = new ir.Expr(
              8,
              [label_cell_expr, label_number_expr],
              undefined
            );
            ...
            for (let i = 0; i < bin.varsToCheckpoint.length; i++) {
              emit_stack.push(
                bin.setMap.get(bin.varsToCheckpoint[i].getName())
              );
            }
            for (let i = 0; i < bin.boolsToCheckpoint.length; i++) {
              emit_stack.push(
                bin.setMap.get(bin.boolsToCheckpoint[i].getName())
              );
            }
            // add arrays
            for (let i = 0; i < bin.arraysToCheckpoint.length; i++) {
              emit_stack.push(
                bin.setMap.get(bin.arraysToCheckpoint[i].getName())
              );
            }
            for (let i = 0; i < bin.stringsToCheckpoint.length; i++) {
              emit_stack.push(
                bin.setMap.get(bin.stringsToCheckpoint[i].getName())
              );
            }
            ...
            if (bin.optimization == 1) {
              ...
              let millis_mul_expr = new ir.Expr(
                3,
                [millis_expr, expr_2],
                "numops::muls"
              );
              let millis_fromInt = fromInt(millis_expr);
              let millis_dub_add_expr = new ir.Expr(
                3,
                [millis_mul_expr, expr_1],
                "numops::adds"
              );
              let debug_expr = new ir.Expr(
                8,
                [timer_cellref_expr, millis_fromInt],
                ""
              );
              let debug_stmt = new ir.Stmt(1, debug_expr);
              let subs_expr = new ir.Expr(
                3,
                [millis_fromInt, timer_cellref_expr],
                "numops::subs"
              );
              let duration_expr = new ir.Expr(
                1,
                null,
                valueEncode(bin.opt_val)
              );
              let numops_gt_expr = new ir.Expr(
                3,
                [subs_expr, duration_expr],
                "numops::gt"
              );
              let gtBool_expr = new ir.Expr(
                3,
                [numops_gt_expr],
                "numops::toBoolDecr"
              );
              ...
              //timer reset
              let timer_store_expr = new ir.Expr(
                8,
                [timer_cellref_expr, millis_fromInt],
                ""
              );
              ...
              let analogread_expr = new ir.Expr(
                3,
                [p1_expr],
                "pins::analogReadPin"
              );
              ...
              let numopts_lt_expr = new ir.Expr(
                3,
                [analogfromint_expr, jitval],
                "numops::lt"
              );
              let toBool_expr = new ir.Expr(
                3,
                [numopts_lt_expr],
                "numops::toBoolDecr"
              );
              ...
              let numops_gt_expr = new ir.Expr(
                3,
                [weight_cellref_expr, val_expr],
                "numops::gt"
              );
              let weightBool_expr = new ir.Expr(
                3,
                [numops_gt_expr],
                "numops::toBoolDecr"
              );
              ...
              let add_expr = new ir.Expr(
                3,
                [weight_cellref_expr, val1_expr],
                "numops::adds"
              );
              let storeadd_expr = new ir.Expr(
                8,
                [weight_cellref_expr, add_expr],
                null
              );
                ...
                if (proc.body[i].expr.args[0].data) {
                  if (proc.body[i].expr.args[0].data.def) {
                    if (proc.body[i].expr.args[0].data.def.name) {
                      if (
                        bin.setMap.has(
                          proc.body[i].expr.args[0].data.def.name.escapedText
                        )
                      ) {
                        if (
                          !emit_set.has(
                            bin.setMap.get(
                              proc.body[i].expr.args[0].data.def.name
                                .escapedText
                            )
                          )
                        ) {
      ...
      for (let i = 0; i < bin.procs[0].body.length; i++) {
        if (bin.procs[0].body[i].stmtKind == ir.SK.Label) {
          if (bin.procs[0].body[i].lblName.includes("fortop")) {
            ...
            let labelid = bin.procs[0].body[i].lblName.substring(
              lblindex,
              lbllength
            );
            ...
            let labelid = bin.procs[0].body[i].lblName.substring(
              bin.procs[0].body[i].lblName.lastIndexOf("."),
              bin.procs[0].body[i].lblName.length
            );
            console.log("LABEL ID ", labelid);
            if (labelMap.has(labelid)) {
              //replace
              let newlabel = bin.procs[0].mkLabel(
                "cont" + labelMap.get(labelid)
              );
              bin.procs[0].body[i] = newlabel;
            } else {
            ...
            bin.procs[0].body[i].lblName.includes("else") ||
            bin.procs[0].body[i].lblName.includes("afterif")
          ) {
      ...
      for (let i = 0; i < proc.body.length; i++) {
        if (proc.body[i].stmtKind == ir.SK.Expr) {
          if (proc.body[i].expr.exprKind == ir.EK.ProcCall) {
            ...
            if (
              insertProcinMain(i, proc.body[i].expr.data.proc.seqNo - 1, false)
            ) {
              console.log(
                "fixing args for proc ",
                bin.procs[proc.body[i].expr.data.proc.seqNo - 1]
              );
              fixArgsProcCall(i);
            }
            ...
            if (proc.body[i].expr.data == "basic::forever") {
              ...
              let inlineID = parseInt(
                inlineName.substring(
                  inlineName.lastIndexOf("P") + 1,
                  inlineName.length
                )
              );
    ...
    function findandFixandRetProcinExpr(
      expr: ir.Expr,
      i: number,
      procnumber: number
    ) {
          ...
          if (expr.args[p].exprKind == ir.EK.ProcCall) {
            if (expr.args[p].args != null) {
              ...
              for (let j = 0; j < expr.args[p].args.length; j++) {
                if (
                  expr.args[p].args[j].exprKind == EK.NumberLiteral ||
                  expr.args[p].args[j].exprKind == EK.CellRef
                ) {
                  ...
                  let assignlitexpr = new ir.Expr(
                    ir.EK.Store,
                    [argexpr, expr.args[p].args[j]],
                    null
                  );
              ...
              bin.procs[0].body.splice(
                i,
                1,
                bin.procs[0].mkLabel("innerprocstart")
              );
    ...
    function fixArgsProcCall(i: number) {
      ...
      if (bin.procs[0].body[i].expr.args != null) {
        ...
        for (let j = 0; j < bin.procs[0].body[i].expr.args.length; j++) {
          if (
            bin.procs[0].body[i].expr.args[j].exprKind == EK.NumberLiteral ||
            bin.procs[0].body[i].expr.args[j].exprKind == EK.CellRef
          ) {
            let argarrexpr = new ir.Expr(ir.EK.CellRef, [], bin.argarr);
            let argposexpr = new ir.Expr(
              ir.EK.NumberLiteral,
              [],
              bin.argarrpos
            );

            let argarrassignexpr = new ir.Expr(
              ir.EK.RuntimeCall,
              [argarrexpr, argposexpr, bin.procs[0].body[i].expr.args[j]],
              "Array_::setAt"
            );
            let argarrstmt = new ir.Stmt(ir.SK.Expr, argarrassignexpr);

            let argarrGetAtExpr = new ir.Expr(
              ir.EK.RuntimeCall,
              [argarrexpr, argposexpr],
              "Array_::getAt"
            );
            ...
            argMap.set(
              bin.procs[0].body[i].expr.data.proc.args[j].getName(),
              argexpr
            );
            let assignlitexpr = new ir.Expr(
              ir.EK.Store,
              [argexpr, bin.procs[0].body[i].expr.args[j]],
              null
            );
            let assignstmt = new ir.Stmt(ir.SK.Expr, assignlitexpr);
    ...
    function insertProcinMain(
      i: number,
      procnumber: number,
      isbasicforever: boolean
    ): boolean {
      console.log("entered insertprocinmain");

      if (
        bin.procs[procnumber].info.decl.parent.getSourceFile().fileName ==
        "main.ts"
      ) {
          ...
          switch (temp_stmt_kind) {
            case 1:
              ...
              bin.procs[0].body.splice(
                i + 1 + j,
                0,
                deepCopyStmt(bin.procs[procnumber].body[j])
              );
              break;
            case 2:
                  ...
                  if (bin.procs[0].body[i].expr.args[1].args) {
                    if (bin.procs[0].body[i].expr.args[1].args[0]) {
                      if (bin.procs[0].body[i].expr.args[1].data) {
                        if (
                          bin.procs[0].body[i].expr.args[1].data ==
                          "pins::createBuffer"
                        ) {
                          bin.procs[0].body[i].expr.args[1].args[0].data =
                            bin.varsToCheckpoint.length * 4 + 1;
                        }
                      }
                    }
                  }
    ...
    function unhandled(n: Node, info?: string, code: number = 9202) {
      ...
      switch (n.kind) {
        ...
        case ts.SyntaxKind.ClassExpression:
          ...
          alternative = lf(
            "declare a class as class C {} not let C = class {}"
          );
          break;
        default:
          break;
    ...
    function getFunctionInfo(f: EmittableAsCall) {
      ...
      if (!info.functionInfo) info.functionInfo = new FunctionAddInfo(f);
    ...
    function recordUse(v: VarOrParam, written = false) {
      ...
      if (varParent == null || varParent == proc.action) {
        ...
        while (curr && curr != varParent) {
          ...
          if (info2.capturedVars.indexOf(v) < 0) info2.capturedVars.push(v);
    ...
    function recordAction<T>(f: (bin: Binary) => T): T {
      const r = f(bin);
      if (needsUsingInfo) bin.recordAction(currUsingContext, f);
      return r;
    }
    ...
    function finalEmit() {
      ...
      for (let proc of bin.procs)
        if (!proc.cachedJS || proc.inlineBody) proc.resolve();

      if (target.isNative)
        bin.procs = bin.procs.filter((p) =>
          p.inlineBody && !p.info.usedAsIface && !p.info.usedAsValue
            ? false
            : true
        );
      ...
      if (opts.target.isNative) {
        if (opts.extinfo.yotta)
          bin.writeFile(
            "yotta.json",
            JSON.stringify(opts.extinfo.yotta, null, 2)
          );
        if (opts.extinfo.platformio)
          bin.writeFile(
            "platformio.json",
            JSON.stringify(opts.extinfo.platformio, null, 2)
          );
        if (opts.target.nativeType == NATIVE_TYPE_VM) vmEmit(bin, opts);
        else processorEmit(bin, opts, res);
      } else {
        jsEmit(bin);
      }
    }
    ...
    function emitGlobal(decl: Declaration) {
      ...
      if (!pinfo.cell) pinfo.cell = new ir.Cell(null, decl, getVarInfo(decl));
      if (bin.globals.indexOf(pinfo.cell) < 0) bin.globals.push(pinfo.cell);
    }
    ...
    function lookupCell(decl: Declaration): ir.Cell {
      if (isGlobalVar(decl)) {
        markUsed(decl);
        const pinfo = pxtInfo(decl);
        if (!pinfo.cell) emitGlobal(decl);
        return pinfo.cell;
      } else {
        let res = proc.localIndex(decl);
        if (!res) {
          if (bin.finalPass) userError(9204, lf("cannot locate identifer"));
          else {
      ...
      for (let ci of bin.usedClassInfos) {
        for (let m of ci.allMethods()) {
          ...
          if (pinfo.flags & PxtNodeFlags.IsUsed) {
            ...
            if (info.virtualParent) markFunctionUsed(info.virtualParent.decl);
    ...
    function getVTable(inf: ClassInfo) {
      ...
      if (inf.vtable) return inf.vtable;
      let tbl = inf.baseClassInfo ? getVTable(inf.baseClassInfo).slice(0) : [];
      inf.derivedClasses = [];
      if (inf.baseClassInfo) inf.baseClassInfo.derivedClasses.push(inf);
      ...
      for (let curr = inf; curr; curr = curr.baseClassInfo) {
        for (let m of curr.usedMethods()) {
          ...
          if (ex) {
            if (isSet && !ex.setProc) ex.setProc = proc;
            else if (isGet && !ex.proc) ex.proc = proc;
            ex.info = 0;
          } else {
            inf.itable.push({
              name: n,
        ...
        if (baseMethod) {
          ...
          if (baseMethod.parameters.length != currMethod.parameters.length)
            error(
              ...
              9255,
              lf(
                "the overriding method is currently required to have the same number of arguments as the base one"
              )
            );
    ...
    function needsFullRecompileIfCached(pxtinfo: PxtNode) {
      if (
        pxtinfo.flags & PxtNodeFlags.FromPreviousCompile ||
    ...
    function getClassInfo(t: Type, decl: ClassDeclaration = null) {
      if (!decl) decl = <ClassDeclaration>t.symbol.valueDeclaration;
      const pinfo = pxtInfo(decl);
      if (!pinfo.classInfo) {
        ...
        for (let mem of decl.members) {
          if (mem.kind == SK.PropertyDeclaration) {
            ...
            if (!isStatic(pdecl)) info.allfields.push(pdecl);
            ...
            for (let p of (mem as FunctionLikeDeclaration).parameters) {
              if (isCtorField(p)) info.allfields.push(p);
            }
          } else if (isClassFunction(mem)) {
            let minf = getFunctionInfo(mem as any);
            minf.parentClassInfo = info;
            if (minf.isUsed) markVTableUsed(info);
            const key = getName(mem);
            if (!info.methods.hasOwnProperty(key)) info.methods[key] = [];
        ...
        switch (s[i]) {
            ...
            if (x) {
              if (w == 0) w = x;
                ...
                userError(
                  9205,
                  lf(
                    "lines in image literal have to have the same width (got {0} and then {1} pixels)",
                    w,
                    x
                  )
                );
          ...
          default:
            userError(
              9206,
              lf(
                "Only 0 . _ (off) and 1 # * (on) are allowed in image literals"
              )
            );
        }
      }
...
`);
    ...
    function isGlobalConst(decl: Declaration) {
      if (isGlobalVar(decl) && decl.parent.flags & NodeFlags.Const) return true;
      return false;
    }
    ...
    function isSideEffectfulInitializer(init: Expression): boolean {
      ...
      switch (init.kind) {
        case SK.ArrayLiteralExpression:
          return (init as ArrayLiteralExpression).elements.some(
    ...
    function emitLocalLoad(decl: VarOrParam) {
      ...
      if (folded) return emitLit(folded.val);
      if (isGlobalVar(decl)) {
        const attrs = parseComments(decl);
        if (attrs.shim) return emitShim(decl, decl, []);
      }
    ...
    function emitFunLiteral(f: FunctionDeclaration) {
      ...
      if (attrs.shim)
        userError(
          9207,
          lf(
            "built-in functions cannot be yet used as values; did you forget ()?"
          )
        );
      ...
      if (info.location) {
        ...
        assert(
          !bin.finalPass || info.capturedVars.length == 0,
          "!bin.finalPass || info.capturedVars.length == 0"
        );
    ...
    function markUsageOrder(info: FunctionAddInfo) {
      if (info.usedBeforeDecl === undefined) info.usedBeforeDecl = true;
      else if (
        bin.finalPass &&
        info.usedBeforeDecl &&
        info.capturedVars.length
      ) {
        if (getEnclosingFunction(info.decl) && !info.alreadyEmitted)
          userError(
            9278,
            lf("function referenced before all variables it uses are defined")
          );
      }
    }
    ...
    function emitIdentifier(node: Identifier): ir.Expr {
      ...
      if (fold) return emitLit(fold.val);
      ...
      if (decl && (isVar(decl) || isParameter(decl))) {
        ...
        if (node.text == "undefined") return emitLit(undefined);
        else
    ...
    function emitThis(node: Node) {
      ...
      if (!meth) userError(9208, lf("'this' used outside of a method"));
    ...
    function asString(e: Node) {
      ...
      let expr = emitExpr(e);
      if (target.isNative || isStringLiteral(e)) return irToNode(expr, isRef);
      expr = ir.rtcallMask(
        "String_::stringConv",
        1,
        ir.CallingConvention.Async,
        [expr]
      );
    ...
    function emitTemplateExpression(node: TemplateExpression) {
      ...
      let concat = (a: ir.Expr, b: Expression | TemplateLiteralFragment) => {
        if (isEmptyStringLiteral(b)) return a;
        numconcat++;
        return rtcallMask(
          "String_::concat",
          [irToNode(a, true), asString(b)],
          null
        );
      };
      ...
      if (numconcat == 0) {
        ...
        return rtcallMask(
          ...
          [
            irToNode(expr, true),
            irToNode(ir.rtcall("String_::mkEmpty", []), false),
          ],
          null
        );
      }
    ...
    function emitObjectLiteral(node: ObjectLiteralExpression) {
      ...
      node.properties.forEach(
        (p: PropertyAssignment | ShorthandPropertyAssignment) => {
          ...
          if (p.kind == SK.ShorthandPropertyAssignment) {
            ...
            const vname: Identifier =
              vsym &&
              vsym.valueDeclaration &&
              (vsym.valueDeclaration as any).name;
            if (vname && vname.kind == SK.Identifier)
              init = emitIdentifier(vname);
            else throw unhandled(p); // not sure what happened
            ...
            proc.emitExpr(
              rtcallMask(
                "pxtrt::mapSetByString",
                [irToNode(expr, true), keyExpr, p.initializer],
                null
              )
            );
            return;
            ...
            keyName =
              p.name.kind == SK.StringLiteral
                ? (p.name as StringLiteral).text
                : p.name.getText();
          ...
          const args = [expr, fieldId, init];
          proc.emitExpr(
            ir.rtcall(
              target.isNative ? "pxtrt::mapSet" : "pxtrt::mapSetByString",
              args
            )
          );
        }
      ...
      if (node.initializer) {
        ...
        if (bin.finalPass && info.isUsed && !info.ctor)
          userError(
            9209,
            lf(
              "class field initializers currently require an explicit constructor"
            )
          );
      }
      // do nothing
    ...
    function emitPropertyAccess(node: PropertyAccessExpression): ir.Expr {
      ...
      if (decl.kind == SK.SetAccessor) decl = checkGetter(decl);
      ...
      if (decl.kind == SK.GetAccessor)
        return emitCallCore(
          ...
          [],
          null,
          decl as GetAccessorDeclaration
        );
      ...
      if (decl.kind == SK.EnumMember) {
        ...
        decl.kind == SK.PropertyDeclaration ||
        ...
        throw userError(
          9211,
          lf("cannot use method as lambda; did you forget '()' ?")
        );
        ...
        throw unhandled(
          node,
          lf("Unknown property access for {0}", stringKind(decl)),
          9237
        );
      }
    }
    ...
    function checkGetter(decl: Declaration) {
      ...
      if (getter == null) {
        throw userError(
          9281,
          lf("setter currently requires a corresponding getter")
        );
      } else {
    ...
    function isSlowField(decl: Declaration) {
      if (decl.kind == SK.Parameter || decl.kind == SK.PropertyDeclaration) {
        const pinfo = pxtInfo(decl);
        return (
          !!target.switches.slowFields ||
          !!(pinfo.flags & PxtNodeFlags.IsOverridden)
        );
      }
      return false;
    }

    function emitIndexedAccess(
      node: ElementAccessExpression,
      assign: Expression = null
    ): ir.Expr {
      ...
      if (
        !indexer &&
        t.flags & (TypeFlags.Any | TypeFlags.StructuredOrTypeVariable)
      ) {
      ...
      if (indexer) {
        if (stringOk || isNumberLike(node.argumentExpression)) {
          ...
          throw unhandled(
            node,
            lf("non-numeric indexer on {0}", indexer),
            9238
          );
        }
      } else {
    ...
    function isOnDemandGlobal(decl: Declaration) {
      if (!isGlobalVar(decl)) return false;
      let v = decl as VariableDeclaration;
      if (!isSideEffectfulInitializer(v.initializer)) return true;
      let attrs = parseComments(decl);
      if (attrs.whenUsed) return true;
      return false;
    }

    function isOnDemandDecl(decl: Declaration) {
      let res =
        isOnDemandGlobal(decl) ||
        isTopLevelFunctionDecl(decl) ||
        isClassDeclaration(decl);
      if (target.switches.noTreeShake) return false;
      if (opts.testMode && res) {
        if (!isInPxtModules(decl)) return false;
      }
      return res;
    }

    function shouldEmitNow(node: Declaration) {
      if (!isOnDemandDecl(node)) return true;
      const info = pxtInfo(node);
      if (bin.finalPass) return !!(info.flags & PxtNodeFlags.IsUsed);
      else return info == currUsingContext;
    }
    ...
    function markUsed(decl: Declaration) {
      ...
      if (!(pinfo.flags & PxtNodeFlags.IsUsed)) {
        ...
        if (isOnDemandDecl(decl)) usedWorkList.push(decl);
      }
    }
    ...
    function getDeclCore(node: Node): Declaration {
      if (!node) return null;
      const pinfo = pxtInfo(node);
      if (pinfo.declCache !== undefined) return pinfo.declCache;
      ...
      if (sym) {
        ...
        if (!decl && sym.declarations) {
          ...
          if (decl0 && decl0.kind == SyntaxKind.ImportEqualsDeclaration) {
            sym = checker.getSymbolAtLocation(
              (decl0 as ImportEqualsDeclaration).moduleReference
            );
            if (sym) decl = sym.valueDeclaration;
          }
        }
      }
      return decl;
    ...
    function isRefCountedExpr(e: Expression) {
      ...
      if (isStringLiteral(e)) return false;
      return true;
    }
    ...
    function emitShim(
      ...
      if (nm.indexOf("(") >= 0) {
        let parse = /(.*)\((.*)\)$/.exec(nm);
        if (parse) {
          if (args.length) U.userError("no arguments expected");
          let litargs: ir.Expr[] = [];
          ...
          if (strargs) {
            for (let a of parse[2].split(/,/)) {
              ...
              if (isNaN(v)) {
                ...
                if (v == null) v = lookupConfigConst(node, a);
                if (v == null)
                  U.userError("invalid argument: " + a + " in " + nm);
              }
              litargs.push(ir.numlit(v));
            }
            if (litargs.length > 4) U.userError("too many args");
          }
          nm = parse[1];
          if (opts.target.isNative) {
            hexfile.validateShim(
              getDeclName(decl),
              nm,
              attrs,
              true,
      ...
      if (nm == "TD_NOOP") {
        ...
        if (target.switches.profile && attrs.shimArgument == "perfCounter") {
          ...
          if (!proc.perfCounterName) proc.perfCounterName = proc.getFullName();
        }
    ...
    function addDefaultParametersAndTypeCheck(
      sig: Signature,
      args: Expression[],
      attrs: CommentAttrs
    ) {
      ...
      if (parms.length > args.length) {
        parms.slice(args.length).forEach((p) => {
          if (p.valueDeclaration && p.valueDeclaration.kind == SK.Parameter) {
            ...
            if (!prm.initializer) {
              ...
              if (!isNumericLiteral(prm.initializer)) {
                userError(
                  9212,
                  lf(
                    "only numbers, null, true and false supported as default arguments"
                  )
                );
              }
            ...
            userError(
              9213,
              lf("unsupported default argument (shouldn't happen)")
            );
          }
        });
      }
      ...
      if (attrs.imageLiteral) {
        if (!isStringLiteral(args[0])) {
          userError(
            9214,
            lf(
              "Only image literals (string literals) supported here; {0}",
              stringKind(args[0])
            )
          );
        }
    ...
    function emitCallCore(
      ...
      if (!decl) decl = getDecl(funcExpr) as EmittableAsCall;
      let hasRecv = false;
      ...
      if (decl) {
        switch (decl.kind) {
          ...
          case SK.MethodSignature:
            ...
            break;
          case SK.Parameter:
            if (isCtorField(decl)) hasRecv = true;
            break;
        ...
        if (funcExpr.kind == SK.PropertyAccessExpression) hasRecv = true; // any-access
      }

      if (target.switches.slowMethods) forceMethod = false;

      const attrs = parseComments(decl);
      let args = callArgs.slice(0);

      if (hasRecv && isStatic(decl)) hasRecv = false;
      ...
      if (res.usedArguments && attrs.trackArgs) {
        ...
        let tracked = attrs.trackArgs
          .map((n) => targs[n])
          .map((e) => {
            let d = getDecl(e);
            if (
              d &&
              (d.kind == SK.EnumMember || d.kind == SK.VariableDeclaration)
            )
        ...
        if (lst.indexOf(tracked) < 0) lst.push(tracked);
      }
      ...
      if (funcExpr.kind == SK.SuperKeyword) {
        ...
        for (
          let p = proc.classInfo.baseClassInfo;
          p && !baseCtor;
          p = p.baseClassInfo
        )
          baseCtor = p.ctor;
        if (!baseCtor && bin.finalPass)
          throw userError(
            9280,
            lf("super() call requires an explicit constructor in base class")
          );
      ...
      if (hasRecv) {
        ...
        if (info.parentClassInfo) markVTableUsed(info.parentClassInfo);
        markFunctionUsed(decl);

        if (recv.kind == SK.SuperKeyword) return emitPlain();

        const needsVCall = !!info.virtualParent;
        const forceIfaceCall =
          !!isStackMachine() || !!target.switches.slowMethods;
        ...
        if (needsVCall && !forceIfaceCall) {
          ...
          U.assert(
            !bin.finalPass || info.virtualIndex != null,
            "!bin.finalPass || info.virtualIndex != null"
          );
          return mkMethodCall(
            args.map((x) => emitExpr(x)),
            ...
            if (sym.name == "helpers") {
              for (let d of sym.declarations || [sym.valueDeclaration]) {
                if (d.kind == SK.ModuleDeclaration) {
                  for (let stmt of (
                    (d as ModuleDeclaration).body as ModuleBlock
                  ).statements) {
      ...
      if (decl && decl.kind == SK.ModuleDeclaration) {
        ...
        else userError(9220, lf("namespaces cannot be called directly"));
      }
    ...
    function mkProcCallCore(
      proc: ir.Procedure,
      callLocation: ts.Node,
      args: ir.Expr[]
    ) {
    ...
    function layOutGlobals() {
      ...
      globals.sort(
        (a, b) => sz(a.bitSize) - sz(b.bitSize) || a.index - b.index
      );
      ...
      for (let g of globals) {
        ...
        while (currOff & (sz - 1)) currOff++; // align
        if (!firstPointer && bitSize == BitSize.None) firstPointer = currOff;
        g.index = currOff;
        currOff += sz;
      }
    ...
    function emitVTables() {
      ...
      const numberClasses = (i: ClassInfo) => {
        U.assert(!i.classNo);
        i.classNo = classNo++;
        for (let subt of i.derivedClasses) if (subt.isUsed) numberClasses(subt);
        i.lastSubtypeNo = classNo - 1;
      };
      for (let info of bin.usedClassInfos) {
        let par = info;
        while (par.baseClassInfo) par = par.baseClassInfo;
        if (!par.classNo) numberClasses(par);
      }
    }

    function getCtor(decl: ClassDeclaration) {
      return decl.members.filter(
        (m) => m.kind == SK.Constructor
      )[0] as ConstructorDeclaration;
    }
    ...
    function emitNewExpression(node: NewExpression) {
      ...
      if (t && isArrayType(t)) {
        ...
        if (ctor) {
          ...
          addDefaultParametersAndTypeCheck(
    ...
    /* Requires the following to be declared in global scope:
        ...
        */
    function emitTaggedTemplateExpression(
            ...
            default:
              ...
              if (v == null) {
                if (attrs.groups.length == 2)
                  ...
                  throw unhandled(
                    node,
                    lf("invalid character in image literal: '{0}'", v),
                    9273
                  );
              }
              line.push(v);
      ...
      function parseHexLiteral(node: Node, s: string) {
          ...
          if (fontMatch) {
            if (!bin.finalPass) {
              ...
              for (let pos = 0; pos < data.length; pos += chsz) {
                let charcode =
                  data.charCodeAt(pos) + (data.charCodeAt(pos + 1) << 8);
                if (
                  charcode < 128 ||
                  mask[charcode >> 5] & (1 << (charcode & 31))
                ) {
            ...
            !thisJres.dataEncoding ||
            thisJres.dataEncoding == "base64"
          ) {
            s = U.toHex(
              U.stringToUint8Array(ts.pxtc.decodeBase64(thisJres.data))
            );
        ...
        if (
          /^e[14]/i.test(s) &&
          node.parent &&
          node.parent.kind == SK.CallExpression &&
          (node.parent as CallExpression).expression.getText() ==
            "image.ofBuffer"
        ) {
        ...
        for (let i = 0; i < s.length; ++i) {
          ...
          if (isHexDigit(c)) {
            ...
            throw unhandled(
              node,
              lf("invalid character in hex literal '{0}'", c),
              9265
            );
        }
        if (target.isNative) {
      ...
      if (!decl) throw unhandled(node, lf("invalid tagged template"), 9265);
      let attrs = parseComments(decl);
      let res: ir.Expr;
      ...
      function handleHexLike(pp: (s: string) => string) {
        res = parseHexLiteral(
          node,
          pp((node.template as ts.LiteralExpression).text)
        );
      }
      ...
      if (node.template.kind != SK.NoSubstitutionTemplateLiteral)
        throw unhandled(
          node,
          lf("substitution not supported in hex literal", attrs.shim),
          9265
        );
      ...
      switch (attrs.shim) {
        ...
        default:
          ...
          throw unhandled(
            node,
            lf("invalid shim '{0}' on tagged template", attrs.shim),
            9265
          );
      }
      if (attrs.helper) {
    ...
    function emitTaggedTemplateHelper(
      ...
      for (let sym of syms) {
        if (sym.name == "helpers") {
          for (let d of sym.declarations || [sym.valueDeclaration]) {
            if (d.kind == SK.ModuleDeclaration) {
              for (let stmt of ((d as ModuleDeclaration).body as ModuleBlock)
                .statements) {
                if (stmt.symbol?.name == attrs.helper) {
                  helperStmt = stmt;
                }
    ...
    function emitFuncCore(node: FunctionLikeDeclaration) {
      ...
      let isExpression =
        node.kind == SK.ArrowFunction || node.kind == SK.FunctionExpression;
      ...
      if (info.usedBeforeDecl === undefined) info.usedBeforeDecl = false;
      ...
      if (caps.length > 0) {
        ...
        lit = ir.shared(
          ir.rtcall("pxt::mkAction", [
            ir.numlit(caps.length),
            emitFunLitCore(node, true),
          ])
        );
        info.usedAsValue = true;
        caps.forEach((l, i) => {
          ...
          if (!loc)
            userError(
              9223,
              lf(
                "cannot find captured value: {0}",
                checker.symbolToString(l.symbol)
              )
            );
          let v = loc.loadCore();
      ...
      for (let p of fieldAssignmentParameters) {
        let idx = fieldIndexCore(
          proc.classInfo,
          getFieldInfo(proc.classInfo, getName(p)),
          false
        );
        let trg2 = ir.op(
          EK.FieldAccess,
          [emitLocalLoad(info.thisParameter)],
          idx
        );
      ...
      for (let f of initalizedFields) {
        let idx = fieldIndexCore(
          proc.classInfo,
          getFieldInfo(proc.classInfo, getName(f)),
          false
        );
        let trg2 = ir.op(
          EK.FieldAccess,
      ...
      if (node.body.kind == SK.Block) {
        ...
        if (funcHasReturn(proc.action)) {
          ...
          if (
            last &&
            last.stmtKind == ir.SK.Jmp &&
            last.jmpMode == ir.JmpMode.Always
          ) {
      ...
      if (
        ...
        if (!vinfo.functionsToDefine) vinfo.functionsToDefine = [];
      ...
      assert(
        !bin.finalPass || usedWorkList.length == 0,
        "!bin.finalPass || usedWorkList.length == 0"
      );
    ...
    function hasShimDummy(node: Declaration) {
      if (opts.target.isNative) return false;
      let f = node as FunctionLikeDeclaration;
      return (
        f.body &&
        (f.body.kind != SK.Block || (f.body as Block).statements.length > 0)
      );
    }

    function emitFunctionDeclaration(node: FunctionLikeDeclaration) {
      if (!shouldEmitNow(node)) return undefined;
      ...
      if (attrs.shim != null) {
        if (attrs.shim[0] == "@") return undefined;
        if (opts.target.isNative) {
          hexfile.validateShim(
            getDeclName(node),
            attrs.shim,
        ...
        if (!hasShimDummy(node)) return undefined;
      }

      if (ts.isInAmbientContext(node)) return undefined;
    ...
    function emitDeleteExpression(node: DeleteExpression) {
      ...
      if (node.expression.kind == SK.PropertyAccessExpression) {
        ...
        throw userError(
          9276,
          lf("expression not supported as argument to 'delete'")
        );
      }
    ...
    function emitPrefixUnaryExpression(node: PrefixUnaryExpression): ir.Expr {
      ...
      switch (node.operator) {
        case SK.ExclamationToken:
          return fromBool(
            ir.rtcall("Boolean_::bang", [emitCondition(node.operand)])
          );
    ...
    function prepForAssignment(trg: Expression, src: Expression = null) {
      ...
      if (
        trg.kind == SK.PropertyAccessExpression ||
        trg.kind == SK.ElementAccessExpression
      ) {
        needsCache((trg as PropertyAccessExpression).expression);
      }
      if (src) needsCache(src);
      if (irCachesToClear.length == prev) return doNothing;
      else
        return () => {
    ...
    function fieldIndexCore(
      info: ClassInfo,
      fld: FieldWithAddInfo,
      needsCheck = true
    ): FieldAccessInfo {
      if (isStatic(fld)) U.oops("fieldIndex on static field: " + getName(fld));
      let attrs = parseComments(fld);
      let idx = info.allfields.indexOf(fld);
      if (idx < 0 && bin.finalPass) U.oops("missing field");
      return {
        idx,
        name: getName(fld),
        isRef: true,
    ...
    function fieldIndex(pacc: PropertyAccessExpression): FieldAccessInfo {
      ...
      if (isPossiblyGenericClassType(tp)) {
        ...
        if (target.switches.noThisCheckOpt) noCheck = false;
        return fieldIndexCore(
          info,
          getFieldInfo(info, pacc.name.text),
          !noCheck
        );
      } else {
    ...
    function getFieldInfo(info: ClassInfo, fieldName: string) {
      const field = info.allfields.filter(
        (f) => (<Identifier>f.name).text == fieldName
      )[0];
      if (!field) {
    ...
    function emitStore(
      ...
      if (trg.kind == SK.Identifier || isGlobal) {
        ...
        if (
          decl &&
          (decl.kind == SK.GetAccessor || decl.kind == SK.SetAccessor)
        ) {
          checkGetter(decl);
          ...
          proc.emitExpr(
            emitCallCore(trg, trg, [src], null, decl as FunctionLikeDeclaration)
          );
        } else if (
          decl &&
          (decl.kind == SK.PropertySignature ||
        ...
        if (src.kind == SK.ArrayLiteralExpression) {
          ...
          (trg as ArrayLiteralExpression).elements.forEach((e, idx) => {
            emitStore(
              e,
              irToNode(
                rtcallMaskDirect("Array_::getAt", [bindingExpr, ir.numlit(idx)])
              )
            );
          });
        }
    ...
    function handleAssignment(node: BinaryExpression) {
      let src = node.right;
      if (node.parent.kind == SK.ExpressionStatement) src = null;
    ...
    function quickGetQualifiedName(expr: Expression): string {
      if (expr.kind == SK.Identifier) {
        ...
        if (left) return left + "." + pa.name.text;
      }
      return null;
    }
    ...
    function fun1Const(expr: Expression, aa: Folded): Folded {
      ...
      switch (quickGetQualifiedName(expr)) {
        case "Math.floor":
          return { val: Math.floor(a) };
        case "Math.ceil":
          return { val: Math.ceil(a) };
        case "Math.round":
          return { val: Math.round(a) };
      }
      return null;
    }
    ...
    function enumValue(decl: EnumMember): string {
      ...
      if (/^[+-]?\d+$/.test(ev)) return ev;
      if (/^0x[A-Fa-f\d]{2,8}$/.test(ev)) return ev;
      U.userError("enumval only support number literals");
      return "0";
    }

    function emitFolded(f: Folded) {
      if (f) return emitLit(f.val);
      return null;
    }
    ...
    function constantFoldDecl(decl: Declaration) {
      ...
      if (info.constantFolded !== undefined) return info.constantFolded;
      ...
      if (isVar(decl) && decl.parent.flags & NodeFlags.Const) {
        ...
        if (ev == null) {
          ...
          const v = parseInt(ev);
          if (!isNaN(v)) info.constantFolded = { val: v };
        }
      } else if (
        decl.kind == SK.PropertyDeclaration &&
        isStatic(decl) &&
    ...
    function emitAsInt(e: Expression) {
      ...
      if (v === undefined)
        throw userError(
          9267,
          lf("a constant number-like expression is required here")
        );
      return v;
    }
    ...
    function lookupConfigConst(ctx: Node, name: string) {
      ...
      if (r == null) r = lookupConfigConstCore(ctx, name, "config");
      return r;
    }

    function lookupConfigConstCore(ctx: Node, name: string, mod: string) {
      let syms = checker.getSymbolsInScope(ctx, SymbolFlags.Module);
      let configMod = syms.filter(
        (s) => s.name == mod && !!s.valueDeclaration
      )[0];
      if (!configMod) return null;
      for (let stmt of (
        (configMod.valueDeclaration as ModuleDeclaration).body as ModuleBlock
      ).statements) {
    ...
    function lookupDalConst(ctx: Node, name: string) {
      ...
      let dalEnm = syms.filter(
        (s) => s.name == "DAL" && !!s.valueDeclaration
      )[0];
      if (!dalEnm) return null;
      let decl = (dalEnm.valueDeclaration as EnumDeclaration).members.filter(
        (s) => s.symbol.name == name
      )[0];
      if (decl) return checker.getConstantValue(decl);
      return null;
    }
    ...
    function isNumberLike(e: Expression) {
      if (e.kind == SK.NullKeyword) {
        ...
        if (vo != null) {
          if (vo.exprKind == EK.NumberLiteral) {
            if (opts.target.isNative) return !!((vo.data as number) & 1);
            return true;
          } else if (
            vo.exprKind == EK.RuntimeCall &&
            vo.data == "pxt::ptrOfLiteral"
          ) {
            if (
              vo.args[0].exprKind == EK.PointerLiteral &&
              !isNaN(parseFloat(vo.args[0].jsInfo as string))
            )
              return true;
            return false;
          } else if (
            vo.exprKind == EK.PointerLiteral &&
      ...
      if (e.kind == SK.NumericLiteral) return true;
    ...
    function rtcallMask(
      ...
      let args2 = args.map((a, i) => {
        let r = emitExpr(a);
        if (!needsNumberConversions()) return r;
        let f = fmt[i + 1];
        ...
        if (!f) {
          ...
          if (f == "D") U.oops("double arguments not yet supported"); // take two words
          // TODO disable F on devices with FPU and hard ABI; or maybe altogether
          // or else, think about using the VFP registers
          if (!isNumber)
            U.userError(
              "argsFmt=...F/D... but argument not a number in " + name
            );
          // mask &= ~(1 << i)
          convInfos.push({
            argIdx: i,
            method: f == "D" ? "pxt::toDouble" : "pxt::toFloat",
          });
          return r;
        } else {
      ...
      let r = ir.rtcallMask(
        name,
        mask,
        attrs ? attrs.callingConvention : ir.CallingConvention.Plain,
        args2
      );
      ...
      if (opts.target.isNative) {
        let f0 = fmt[0];
        if (f0 == "I") r = fromInt(r);
        else if (f0 == "B") r = fromBool(r);
        else if (f0 == "F") r = fromFloat(r);
        else if (f0 == "D") {
    ...
    function emitInstanceOfExpression(node: BinaryExpression) {
      ...
      let classDecl = isPossiblyGenericClassType(tp)
        ? <ClassDeclaration>getDecl(node.right)
        : null;
    ...
    function emitBrk(node: Node) {
      ...
      if (!needsComment && !needsBreak) return;
      const src = getSourceFileOfNode(node);
      if (opts.justMyCode && isInPxtModules(src)) needsBreak = false;
      if (!needsComment && !needsBreak) return;
      let pos = node.pos;
      while (/^\s$/.exec(src.text[pos])) pos++;
      ...
      if (needsComment) {
        let endpos = node.end;
        if (endpos - pos > 80) endpos = pos + 80;
        ...
        proc.emit(
          ir.comment(
            `${src.fileName.replace(/pxt_modules\//, "")}(${p.line + 1},${
              p.character + 1
            }): ${srctext}`
          )
        );
    ...
    function emitBinaryExpression(node: BinaryExpression): ir.Expr {
      ...
      if (
        node.operatorToken.kind == SK.PlusToken ||
        node.operatorToken.kind == SK.PlusEqualsToken
      ) {
        lt = typeOf(node.left);
        rt = typeOf(node.right);
        if (
          isStringType(lt) ||
          (isStringType(rt) && node.operatorToken.kind == SK.PlusToken)
        ) {
      ...
      if (node.operatorToken.kind == SK.CommaToken) {
        if (isNoopExpr(node.left)) return emitExpr(node.right);
        else {
      ...
      if (node.operatorToken.kind == SK.PlusToken) {
        if (isStringType(lt) || isStringType(rt)) {
          return rtcallMask(
            "String_::concat",
            [asString(node.left), asString(node.right)],
            null
          );
        }
      }
      ...
      if (noEq) return emitIncrement(node.left, shimName, false, node.right);
    ...
    function checkForLetOrConst(declList: VariableDeclarationList): boolean {
      ...
      throw userError(
        9260,
        lf("variable needs to be defined using 'let' instead of 'var'")
      );
    }
    function emitVariableStatement(node: VariableStatement) {
      function addConfigEntry(ent: ConfigEntry) {
        ...
        if (entry.value != ent.value)
          throw userError(
            9269,
            lf("conflicting values for config.{0}", ent.name)
          );
      }

      if (node.declarationList.flags & NodeFlags.Const) {
        let parname =
          node.parent && node.parent.kind == SK.ModuleBlock
        ...
        if (parname == "config" || parname == "userconfig")
          for (let decl of node.declarationList.declarations) {
            ...
            if (parname == "userconfig") nm = "!" + nm;
            addConfigEntry({ name: nm, key: key, value: val });
          }
      }

      if (ts.isInAmbientContext(node)) return;
    ...
    function emitCondition(expr: Expression, inner: ir.Expr = null) {
      if (!inner && isThumb() && expr.kind == SK.BinaryExpression) {
        ...
        let mapped = U.lookup(
          thumbCmpMap,
          simpleInstruction(be, be.operatorToken.kind)
        );
        if (mapped) {
          return ir.rtcall(mapped, [emitExpr(be.left), emitExpr(be.right)]);
        }
      }
      if (!inner) inner = emitExpr(expr);
      if (isStackMachine()) return inner;
    ...
    function emitIfStatement(node: IfStatement) {
      ...
      if (node.elseStatement) emit(node.elseStatement);
    ...
    function emitForStatement(node: ForStatement) {
      if (
        node.initializer &&
        node.initializer.kind == SK.VariableDeclarationList
      ) {
    ...
    function emitForOfStatement(node: ForOfStatement) {
      if (
        ...
        unhandled(
          node,
          "only a single variable may be used to iterate a collection"
        );
        return;
      }
      ...
      if (iterVar)
        proc.emitExpr(
          iterVar.storeByRef(
            ir.rtcall(indexer, [
              collectionVar.loadCore(),
              toInt(intVarIter.loadCore()),
            ])
          )
        );
    ...
    function emitBreakOrContinueStatement(node: BreakOrContinueStatement) {
      ...
      function findOuter(parent: Node): Statement {
        ...
        if (
          label &&
          parent.kind == SK.LabeledStatement &&
      ...
      if (!stmt) error(node, 9230, lf("cannot find outer loop"));
      else {
    ...
    function emitSwitchStatement(node: SwitchStatement) {
      ...
      let lbls = node.caseBlock.clauses.map((cl) => {
        ...
        if (cl.kind == SK.CaseClause) {
          ...
          let cmpCall = ir.rtcallMask(
            mapIntOpName("pxt::switch_eq"),
            mask,
            ir.CallingConvention.Plain,
            [cmpExpr, expr]
          );
      ...
      if (defaultLabel) proc.emitJmp(defaultLabel, expr);
      else proc.emitJmp(l.brk, expr);
    ...
    function emitThrowStatement(node: ThrowStatement) {
      emitBrk(node);
      proc.emitExpr(
        rtcallMaskDirect("pxt::throwValue", [emitExpr(node.expression)])
      );
    }
    ...
    function emitTryStatement(node: TryStatement) {
      ...
      if (node.finallyBlock) proc.emitExpr(beginTry(lfinally));
      if (node.catchClause) proc.emitExpr(beginTry(lcatch));
      ...
      if (node.catchClause) {
        ...
        if (decl) {
          ...
          proc.emitExpr(
            loc.storeByRef(rtcallMaskDirect("pxt::getThrownValue", []))
          );
        }
        flushHoistedFunctionDefinitions();
    ...
    function inLoop(node: Node) {
      while (node) {
        if (isLoop(node)) return true;
        node = node.parent;
      }
      return false;
    }

    function emitVarOrParam(
      node: VarOrParam,
      bindingExpr: ir.Expr,
      bindingType: Type
    ): ir.Cell {
      if (
        node.name.kind === SK.ObjectBindingPattern ||
        node.name.kind == SK.ArrayBindingPattern
      ) {
        if (!bindingExpr) {
          bindingExpr = node.initializer
            ? ir.shared(emitExpr(node.initializer))
            : emitLocalLoad(node as VariableDeclaration);
          bindingType = node.initializer
            ? typeOf(node.initializer)
            : typeOf(node);
        }
        (node.name as ObjectBindingPattern).elements.forEach(
          (e: BindingElement) => emitVarOrParam(e, bindingExpr, bindingType)
        );
      ...
      if (constantFoldDecl(node)) return null;
      ...
      if (node.kind === SK.BindingElement) {
        emitBrk(node);
        let [expr, tp] = bindingElementAccessExpression(
        ...
        if (isGlobalVar(node)) {
          ...
          if (jrname) {
            ...
            if (!jr) {
              userError(
                9270,
                lf("resource '{0}' not found in any .jres file", jrname)
              );
            } else {
              currJres = jr;
            }
    ...
    function emitFieldAccess(
      ...
      if (isPossiblyGenericClassType(objType)) {
        ...
        exres = ir.op(
          EK.FieldAccess,
          [objRef],
          fieldIndexCore(info, getFieldInfo(info, fieldName))
        );
      } else {
    ...
    function bindingElementAccessExpression(
      bindingElement: BindingElement,
      parentAccess: ir.Expr,
      parentType: Type
    ): [ir.Expr, Type] {
      const target = bindingElement.parent.parent;

      if (target.kind === SK.BindingElement) {
        const parent = bindingElementAccessExpression(
          target as BindingElement,
          parentAccess,
          parentType
        );
      ...
      if (bindingElement.parent.kind == SK.ArrayBindingPattern) {
        const idx = (
          bindingElement.parent as ArrayBindingPattern
        ).elements.indexOf(bindingElement);
        ...
        const propertyName = (bindingElement.propertyName ||
          bindingElement.name) as Identifier;
        return emitFieldAccess(
          bindingElement,
          parentAccess,
          parentType,
          propertyName.text
    ...
    function emitInterfaceDeclaration(node: InterfaceDeclaration) {
      ...
      let attrs = parseComments(node);
      if (attrs.autoCreate) autoCreateFunctions[attrs.autoCreate] = true;
    }
    ...
    function emitTopLevel(node: Declaration): void {
      ...
      if (pinfo.usedNodes) {
        needsUsingInfo = false;
        for (let node of U.values(pinfo.usedNodes)) markUsed(node);
        for (let fn of pinfo.usedActions) fn(bin);
        needsUsingInfo = true;
  ...
  export class Binary {
    ...
    getTitle() {
      ...
      if (title.length >= 90) return title.slice(0, 87) + "...";
      else return title;
    }
    ...
    recordHelper(usingCtx: PxtNode, id: string, gen: (bin: Binary) => string) {
      const act = (bin: Binary) => {
        if (!bin.codeHelpers[id]) bin.codeHelpers[id] = gen(bin);
      };
      act(this);
    ...
    recordAction<T>(usingCtx: PxtNode, f: (bin: Binary) => T) {
      if (usingCtx) {
        if (usingCtx.usedActions) usingCtx.usedActions.push(f as EmitAction);
      } else U.oops("no using ctx!");
    }
    ...
    emitDouble(v: number): string {
      return this.emitLabelled(
        target.switches.numFloat ? floatToBits(v) : doubleToBits(v),
        this.doubles,
        "_dbl"
      );
    }
    ...
    emitString(s: string): string {
      if (!this.finalPass)
        for (let i = 0; i < s.length; ++i) {
          ...
          if (ch >= 128) this.usedChars[ch >> 5] |= 1 << (ch & 31);
        }
    ...
    setPerfCounters(systemPerfCounters: string[]) {
      if (!target.switches.profile) return [];
  ...
  export function isCtorField(p: ParameterDeclaration) {
    ...
    if (p.parent.kind != SK.Constructor) return false;
    for (let m of p.modifiers) {
      if (
        m.kind == SK.PrivateKeyword ||
  ...
  function isNumberLikeType(type: Type): boolean {
    if (type.flags & TypeFlags.Union) {
      ...
      return !!(
        type.flags &
        (TypeFlags.NumberLike | TypeFlags.EnumLike | TypeFlags.BooleanLike)
      );
    }
  }
}
